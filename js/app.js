...
// Função global para normalizar o valor da célula do Dígito (GBZ - v1.2.40)
...
// Função global para copiar número do processo (SEI) com feedback visual imediato (GBZ - v1.2.40)
...
window.popularDigitosDisponiveis = function() {
  ...
  if (container) {
    if (distinctDigitos.length === 0) {
      ...
      // Removida a palavra DÍGITO do dropbox, mantendo somente o valor real da célula (GBZ - v1.2.40)
...
async function carregarPainelSistemaInfo() {
  ...
  // Renderizar tabela de conexões/usuários com detecção de usuários ativos em tempo real (GBZ - v1.2.40)
  ...
  const renderTabelaUsuarios = (lista) => {
    ...
    tbodyLogados.innerHTML = ordenados.map((u) => {
      ...
      let statusBadge = '';
      if (isCurrent) {
        // Destaque amarelo ouro exclusivo para Você / Elton (GBZ - v1.2.40)
