...
app.delete("/api/contatos/:id", editorOnly, async (req, res) => {
  try {
    ...
    if (!sheet) return res.status(404).json({ erro: "Aba contatos não encontrada." });
...
if (!content.includes('/api/contatos')) {
  content = content.replace(/(async function garantirColunasAdicionais\(\) {)/, jsContent + '\n$1');
  fs.writeFileSync(targetFile, content, 'utf8');
  console.log("Endpoints de contatos inseridos com sucesso.");
} else {
