...
const startIdx = content.indexOf('window.verificarInconsistenciasPlanilha = function() {');
const endMarker = 'recarregarDadosGlobais();\n  }\n};';
...
window.verificarInconsistenciasPlanilha = function() {
  ...
  if (inconsistentes.length === 0) {
    ...
    htmlTable += '<tr style="border-bottom:1px solid rgba(255,255,255,0.2);"><th style="padding:4px; text-align:left;">Processo</th><th style="padding:4px; text-align:left;">Status (Antes -> Depois)</th><th style="padding:4px; text-align:left;">Localizaǜo (Antes -> Depois)</th></tr>';
...
window.executarPadronizacaoPlanilha = async function() {
  ...
  fs.writeFileSync('js/app.js', before + newLogic + after);
