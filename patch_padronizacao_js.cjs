...
const regex = /window\.verificarInconsistenciasPlanilha = function\(\) \{[\s\S]*?\} catch \(e\) \{\s*console\.error\(e\);\s*\}\s*\};/g;
...
window.verificarInconsistenciasPlanilha = function() {
  ...
  if (inconsistentes.length === 0) {
    ...
    if (logDiv) logDiv.innerHTML = '<div>Sem erros ortogrǭficos ou de espaamento.</div>';
  } else {
    if (labelStatus) labelStatus.textContent = \`Atenǜo: \${inconsistentes.length} divergǦncias encontradas. Por favor, autorize as mudanas abaixo:\`;
    
    let htmlTable = '<table style="width:100%; border-collapse:collapse; margin-top:10px;">';
    htmlTable += '<tr style="border-bottom:1px solid rgba(255,255,255,0.2);"><th style="padding:4px; text-align:left;">Processo</th><th style="padding:4px; text-align:left;">Status (Antes s> Depois)</th><th style="padding:4px; text-align:left;">Localizaǜo (Antes s> Depois)</th></tr>';
    ...
    if (btnExecutar) btnExecutar.style.display = 'flex'; // show confirmation button
  }
};
...
window.executarPadronizacaoPlanilha = async function() {
  ...
  try {
    ...
    // We send only the diffs to a bulk update endpoint (if it existed)
    // But since we only have single update, we might have to do it one by one, OR we just use a trick.
    // Wait, the GAS script has updateProcesso endpoint. Let's do it sequentially but fast.
    ...
    if (labelStatus) labelStatus.textContent = '✓ Correes aplicadas com sucesso!';
    setTimeout(() => {
...
console.log('patched padronizacao JS');
