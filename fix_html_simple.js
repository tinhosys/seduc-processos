...
const regex = /(<input[^>]*id="form-banheiros"[^>]*>.*?)<textarea[^>]*id="form-detalhamentoItens"/s;
const match = html.match(regex);
if (match) {
    const replacement = `<input type="text" id="form-banheiros" class="form-control" placeholder="Ex: Masc / Fem / Acessível" style="border-color:rgba(16,185,129,0.4); background:rgba(0,0,0,0.3); color:#fff; font-weight:600; padding:10px 12px;">
                </div>
              </div>
              ...
              <div style="margin-top:24px; padding:20px; background:linear-gradient(135deg, rgba(59,130,246,0.08), rgba(139,92,246,0.05)); border:1px solid rgba(59,130,246,0.3); border-radius:12px; margin-bottom:20px;">
                ...
                <div class="form-group">
                  ...
                  <textarea id="form-detalhamentoItens"`;
    
    html = html.replace(regex, replacement);
    html = html.replace('GBZ - v1.0.0', 'GBZ - v1.0.1'); // update version
    fs.writeFileSync('index.html', html);
    console.log("Fixed HTML correctly");
} else {
