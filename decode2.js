...
const files = ['index.html', 'js/mapa.js', 'js/escolas.js'];

files.forEach(f => {
    if (fs.existsSync(f)) {
        let str = fs.readFileSync(f, 'utf8');
        // We will build an array of bytes
        let bytes = [];
        for (let i = 0; i < str.length; i++) {
            ...
            if (win1252ToByte[code] !== undefined) {
                ...
                // If it's an emoji (e.g. surrogate pair), it is > 255.
                // We convert it to its UTF-8 representation and push those bytes.
                let charStr = str.charAt(i);
                if (code >= 0xD800 && code <= 0xDBFF && i + 1 < str.length) {
                    // surrogate pair
                    charStr += str.charAt(++i);
                }
                let buf = Buffer.from(charStr, 'utf8');
