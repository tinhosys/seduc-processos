...
let safeReplacement = replacement.replace(/\${/g, '\\${'); // Wait! If I do this, it will write `\${` in the file.
// Since I'm using `fs.writeFileSync(file, content.replace...)` where replacement is just a JS string, it will literal write `${` if I don't use template literals inside the script block! Wait, `let replacement = \`...\`` in node.js eval will interpolate them! So I must escape them IN node script string.
// Yes, I escaped them by doing \\${. Let's make sure.

fs.writeFileSync(file, content.substring(0, start) + replacement.replace(/\\\${/g, '${') + content.substring(end));
