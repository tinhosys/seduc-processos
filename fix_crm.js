...
const crmStart = js.indexOf('// ============================================================');
if (crmStart > -1) {
  js = js.substring(0, crmStart);
}
...
async function carregarCRM() {
  try {
    ...
    // Use GViz endpoint to avoid CORS and redirects!
    ...
    if (!response.ok) throw new Error('Falha na resposta HTTP: ' + response.status);
    const text = await response.text();
    
    const lines = text.split('\\n').map(l => l.trim()).filter(l => l);
    
    _crmData = lines.slice(1).map(line => {
      // GViz CSV quotes every field. We can parse it easily:
      // match "something" or empty between commas
      const regex = /"([^"]*)"|([^,]+)/g;
      const row = [];
      let m;
    ...
    document.getElementById('orc-crm-container').innerHTML = '<div style="padding: 20px; color: #ef4444;">Erro ao carregar dados do CRM. Verifique se a planilha est\u00E1 p\u00FAblica ou verifique sua conex\u00E3o.</div>';
  }
}
...
console.log('Fixed CRM logic injected.');

// Bump version in index.html
let html = fs.readFileSync('index.html', 'utf8');
html = html.replace(/v1\.1\.06/g, 'v1.1.07');
fs.writeFileSync('index.html', html, 'utf8');
console.log('Version bumped to v1.1.07');
