...
`;
if (!js.includes('filtrarOrcamentoPorNatureza')) {
  js = js.replace('window.filtrarOrcamento       = filtrarOrcamento;', filterFn + '\nwindow.filtrarOrcamento       = filtrarOrcamento;');
}

// 2. Column widths fix
js = js.replace(
  `<td style="padding:10px 12px; font-size:12px; font-weight:700; color:#60a5fa; white-space:nowrap;">
          <div>\${row.pa}</div>
          <div style="font-weight:400; font-size:10px; color:#64748b; margin-top:2px;">\${paDesc}</div>
        </td>`,
...
js = js.replace(
  `<td style="padding:10px 12px; font-size:11px; color:#94a3b8; white-space:nowrap;">
          <div>\${row.fonte}</div>
          <div style="font-size:10px; color:#64748b;">\${fonteDesc.split('–')[0].trim()}</div>
        </td>`,
...
js = js.replace(
  `<td style="padding:10px 12px; font-size:12px; color:#e2e8f0; max-width:220px;">
          <div style="font-weight:600;">\${nat?.nome || row.despesa}</div>
          <div style="font-size:10px; color:#64748b; margin-top:2px;">\${row.detalhamento.length > 55 ? row.detalhamento.slice(0,55)+'…' : row.detalhamento}</div>
        </td>`,
...
// Also the nat icon 
js = js.replace(`<td style="padding:10px 12px; white-space:nowrap;">`, `<td style="padding:10px 12px; width:90px; min-width:90px; white-space:nowrap;">`);

// 3. The CRM logic properly appended (using safe JSON strings and no truncation!)
const crmCode = `
...
function classificarProcesso(p) {
  ...
  if (t.includes('PROALFA') || t.includes('ALFABETIZA\\u00C7\\u00C3O') || p.pa === '4097') return 'PROALFA';
  if (t.includes('SAERO') || t.includes('AVALIA\\u00C7\\u00C3O') || p.pa === '4100') return 'GMAC';
...
function renderCRM() {
  ...
  ['PROALFA', 'GMAC', 'GDSM', 'CAM'].forEach(grupo => {
    ...
    const totalValor = procs.reduce((acc, p) => {
      ...
      if (p.tipo === 'Anula\\u00E7\\u00E3o') return acc - v;
      return acc + v;
    }, 0);
            ...
            <table style="width: 100%; border-collapse: collapse; font-size: 12px; text-align: left;">
              ...
              <tbody>
                \${procs.map(p => {
                  ...
                  if(p.tipo === 'Anula\\u00E7\\u00E3o') badgeCor = '#f87171';
                  ...
                  return \`
                    <tr style="border-bottom: 1px solid rgba(255,255,255,0.04); transition: background 0.15s;" onmouseover="this.style.background='rgba(255,255,255,0.02)'" onmouseout="this.style.background='transparent'">
                      ...
                      <td style="padding: 10px 16px; text-align: right; font-family: monospace; font-weight: 700; color: \${p.tipo === 'Anula\\u00E7\\u00E3o' ? '#f87171' : '#10b981'}; white-space: nowrap;">
                        \${p.tipo === 'Anula\\u00E7\\u00E3o' ? '-' : ''} R$ \${p.valorText}
                      </td>
                    </tr>
                  \`;
                }).join('')}
...
if (!js.includes('function carregarCRM()')) {
  js = js + crmCode;
}

// 4. Inject carregarCRM() into carregarOrcamento()
if (!js.includes('carregarCRM();')) {
  js = js.replace(
...
console.log('Final rebuild of orcamento.js complete. Length:', js.length);

let html = fs.readFileSync('index.html', 'utf8');
html = html.replace(/v1\.1\.07/g, 'v1.1.08');
fs.writeFileSync('index.html', html, 'utf8');
console.log('Version bumped to 1.1.08');
