...
// The functions start at `window.verificarInconsistenciasPlanilha = function() {`
// And end right before `// ---- PREENCHIMENTO DO MODAL ----` maybe?
// Let's just find the exact block using string matching.

const p1 = 'window.verificarInconsistenciasPlanilha = function() {';
const p2 = 'if (typeof recarregarDadosGlobais === \\'function\\') {\\n    recarregarDadosGlobais();\\n  }\\n};';

const startIdx = content.indexOf(p1);
const endIdx = content.indexOf('recarregarDadosGlobais();', startIdx);
if (startIdx > -1 && endIdx > -1) {
  // find the end of the block '};\n'
  const finalIdx = content.indexOf('};', endIdx) + 2;
  
  const before = content.substring(0, startIdx);
  const after = content.substring(finalIdx);
